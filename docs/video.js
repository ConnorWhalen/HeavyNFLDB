...
function videoPlayer(video, video_player) {
    if(Hls.isSupported()) {
        const hls = new Hls();
        hls.loadSource(`https://assets.heavynfldb.ca/video_streams/${video.id}/playlist.m3u8`);
        ...
        video_player.src = `https://assets.heavynfldb.ca/video_streams/${video.id}/playlist.m3u8`;
    ...
    li.innerHTML = (
        '<span class="song-li-left">' +
            '<div class="song-li-thumb-cover">' +
                `<img class="song-li-thumb" src="https://assets.heavynfldb.ca/thumbnails/${video.id}/hqdefault.jpg"/>` +
            '</div>' +
        '</span>' +
...
function createVideoPreview(video) {
    ...
    preview.innerHTML = (
        ...
        `<div class="song-preview-thumb-cover">` +
            `<img class="song-preview-thumb" src="https://assets.heavynfldb.ca/thumbnails/${video.id}/hqdefault.jpg"></img>` +
        '</div>'
    );
