...
function videoPlayer(video, video_player) {
    if(Hls.isSupported()) {
        const hls = new Hls();
        hls.loadSource(`/static/video_streams/${video.id}/playlist.m3u8`);
        ...
        video_player.src = `/static/video_streams/${video.id}/playlist.m3u8`;
    ...
    li.innerHTML = (
        '<span class="song-li-left">' +
            '<div class="song-li-thumb-cover">' +
                `<img class="song-li-thumb" src="/static/thumbnails/${video.id}/hqdefault.jpg"/>` +
            '</div>' +
        '</span>' +
...
function createVideoPreview(video) {
    ...
    preview.innerHTML = (
        ...
        `<div class="song-preview-thumb-cover">` +
            `<img class="song-preview-thumb" src="/static/thumbnails/${video.id}/hqdefault.jpg"></img>` +
        '</div>'
    );
